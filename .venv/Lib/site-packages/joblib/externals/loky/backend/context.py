...
from concurrent.futures.process import _MAX_WINDOWS_WORKERS #type: ignore
...
def get_context(method=None): #type: ignore
    # Try to overload the default context
    method = method or _DEFAULT_START_METHOD or "loky" #type: ignore
    if method == "fork":
    ...
    try:
        return mp_get_context(method) #type: ignore
    except ValueError:
        raise ValueError(
...
def set_start_method(method, force=False): #type: ignore
    ...
    _DEFAULT_START_METHOD = method #type: ignore


def get_start_method(): #type: ignore
    return _DEFAULT_START_METHOD #type: ignore


def cpu_count(only_physical_cores=False): #type: ignore
...
def _cpu_count_cgroup(os_cpu_count): #type: ignore
    ...
    if cpu_quota_us == "max":
        ...
        return os_cpu_count #type: ignore
    else:
...
def _cpu_count_affinity(os_cpu_count): #type: ignore
    ...
    try:
        ...
        if hasattr(p, "cpu_affinity"):
            return len(p.cpu_affinity()) #type: ignore
...
def _cpu_count_user(os_cpu_count): #type: ignore
    """Number of user defined available CPUs"""
    cpu_count_affinity = _cpu_count_affinity(os_cpu_count) #type: ignore

    cpu_count_cgroup = _cpu_count_cgroup(os_cpu_count) #type: ignore

    # User defined soft-limit passed as a loky specific environment variable.
    cpu_count_loky = int(os.environ.get("LOKY_MAX_CPU_COUNT", os_cpu_count)) #type: ignore

    return min(cpu_count_affinity, cpu_count_cgroup, cpu_count_loky) #type: ignore
...
class LokyContext(BaseContext):
    ...
    _name = "loky"
    Process = LokyProcess
    cpu_count = staticmethod(cpu_count) #type: ignore

    def Queue(self, maxsize=0, reducers=None): #type: ignore
        """Returns a queue object"""
        from .queues import Queue

        return Queue(maxsize, reducers=reducers, ctx=self.get_context())

    def SimpleQueue(self, reducers=None): #type: ignore
        """Returns a queue object"""
...
mp.context._concrete_contexts["loky"] = ctx_loky #type: ignore
mp.context._concrete_contexts["loky_init_main"] = LokyInitMainContext() #type: ignore
