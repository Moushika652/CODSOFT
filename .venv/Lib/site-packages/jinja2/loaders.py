...
def split_template_path(template: str) -> t.List[str]:
    ...
    for piece in template.split("/"):
        if (
            ...
            pieces.append(piece) #type: ignore
    return pieces #type: ignore
...
class BaseLoader:
    ...
    def load(
        ...
        if bcc is not None and bucket.code is None: #type: ignore
            bucket.code = code #type: ignore
            bcc.set_bucket(bucket) #type: ignore
            ...
            for dirpath, _, filenames in walk_dir:
                for filename in filenames:
                    ...
                    if template not in found:
                        found.add(template) #type: ignore
...
class PackageLoader(BaseLoader):
    ...
    def __init__(
        ...
        if isinstance(loader, zipimport.zipimporter):
            ...
            template_root = os.path.join(pkgdir, package_path).rstrip(os.path.sep) #type: ignore
        else:
            roots: t.List[str] = []
    ...
    def get_source(
        ...
        up_to_date: t.Optional[t.Callable[[], bool]] #type: ignore
        ...
        return source.decode(self.encoding), p, up_to_date #type: ignore
    ...
    def list_templates(self) -> t.List[str]:
        ...
        for prefix, loader in self.mapping.items():
            for template in loader.list_templates():
                result.append(prefix + self.delimiter + template) #type: ignore
...
class ChoiceLoader(BaseLoader):
    ...
    def list_templates(self) -> t.List[str]:
        found = set() #type: ignore
        for loader in self.loaders:
            found.update(loader.list_templates()) #type: ignore
        return sorted(found) #type: ignore
