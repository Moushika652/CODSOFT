...
from .async_utils import async_variant #type: ignore
...
if t.TYPE_CHECKING:
    ...
    from .sandbox import SandboxedEnvironment  # noqa: F401 #type: ignore

    class HasHTML(te.Protocol):
        def __html__(self) -> str: #type: ignore
            pass
...
def do_urlencode(
    ...
    .. versionadded:: 2.7
    """
    if isinstance(value, str) or not isinstance(value, abc.Iterable): #type: ignore
        return url_quote(value)

    if isinstance(value, dict):
        items: t.Iterable[t.Tuple[str, t.Any]] = value.items() #type: ignore
    else:
...
def do_items(value: t.Union[t.Mapping[K, V], Undefined]) -> t.Iterator[t.Tuple[K, V]]:
    ...
    if not isinstance(value, abc.Mapping): #type: ignore
...
def do_xmlattr(
    ...
    **Values** that are neither ``none`` nor ``undefined`` are automatically
    ...
    for key, value in d.items():
        ...
        items.append(f'{escape(key)}="{escape(value)}"') #type: ignore

    rv = " ".join(items) #type: ignore
...
def do_filesizeformat(value: t.Union[str, float, int], binary: bool = False) -> str:
    ...
    if bytes == 1:
        ...
        return f"{base * bytes / unit:.1f} {prefix}" #type: ignore
...
def do_urlize(
    ...
    for scheme in extra_schemes: #type: ignore
...
def do_truncate(
    ...
    assert leeway >= 0, f"expected leeway >= 0, got {leeway}" #type: ignore

    if len(s) <= length + leeway: #type: ignore
        return s
...
class _GroupTuple(t.NamedTuple):
    ...
    def __repr__(self) -> str:
        return tuple.__repr__(self) #type: ignore

    def __str__(self) -> str:
        return tuple.__str__(self) #type: ignore
...
async def do_sum(
    ...
    if attribute is not None:
        func = make_attrgetter(environment, attribute) #type: ignore
    else:
...
def prepare_map(
    ...
    if not args and "attribute" in kwargs:
        ...
        func = make_attrgetter(context.environment, attribute, default=default) #type: ignore
    else:
        try:
            name = args[0]
            args = args[1:]
...
def prepare_select_or_reject(
    ...
    if lookup_attr:
        ...
        transfunc = make_attrgetter(context.environment, attr) #type: ignore
        off = 1
    else:
        off = 0
