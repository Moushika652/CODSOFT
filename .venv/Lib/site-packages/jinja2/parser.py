...
class Parser:
    ...
    def __init__(
        ...
        self.stream = environment._tokenize(source, name, filename, state) #type: ignore
        self.name = name
    ...
    def parse_from(self) -> nodes.FromImport:
        ...
        while True:
            ...
            if self.stream.current.type == "name":
                ...
                if parse_context() or self.stream.current.type != "comma": #type: ignore
                    break
            else:
    ...
    def parse_compare(self) -> nodes.Expr:
        ...
        while True:
            ...
            if token_type in _compare_operators:
                next(self.stream)
                ops.append(nodes.Operand(token_type, self.parse_math1())) #type: ignore
            elif self.stream.skip_if("name:in"):
                ops.append(nodes.Operand("in", self.parse_math1())) #type: ignore
            elif self.stream.current.test("name:not") and self.stream.look().test(
                "name:in"
            ):
                self.stream.skip(2)
                ops.append(nodes.Operand("notin", self.parse_math1())) #type: ignore
            else:
                break
            lineno = self.stream.current.lineno
        if not ops:
    ...
    def parse_subscribed(self) -> nodes.Expr:
        ...
        if self.stream.current.type == "colon":
            next(self.stream)
            if self.stream.current.type not in ("rbracket", "comma"): #type: ignore
                args.append(self.parse_expression())
            else:
                args.append(None)
        else:
        ...
        while self.stream.current.type != "rparen":
            ...
            if self.stream.current.type == "mul":
                ...
                if (
                    ...
                    self.stream.skip(2)
                    value = self.parse_expression()
                    kwargs.append(nodes.Keyword(key, value, lineno=value.lineno)) #type: ignore
                else:
                    # Parsing an arg
                    ensure(dyn_args is None and dyn_kwargs is None and not kwargs)
                    args.append(self.parse_expression()) #type: ignore

            require_comma = True

        self.stream.expect("rparen")
        return args, kwargs, dyn_args, dyn_kwargs #type: ignore
