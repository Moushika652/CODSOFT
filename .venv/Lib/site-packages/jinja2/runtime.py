...
from markupsafe import escape  # noqa: F401 #type: ignore
from markupsafe import Markup
from markupsafe import soft_str

from .async_utils import auto_aiter
from .async_utils import auto_await  # noqa: F401 #type: ignore
from .exceptions import TemplateNotFound  # noqa: F401 #type: ignore
from .exceptions import TemplateRuntimeError  # noqa: F401
from .exceptions import UndefinedError
from .nodes import EvalContext
from .utils import _PassArg #type: ignore
from .utils import concat
from .utils import internalcode
from .utils import missing
from .utils import Namespace  # noqa: F401 #type: ignore
...
def markup_join(seq: t.Iterable[t.Any]) -> str:
    ...
    for arg in iterator:
        buf.append(arg) #type: ignore
        if hasattr(arg, "__html__"):
            return Markup("").join(chain(buf, iterator)) #type: ignore
    return concat(buf) #type: ignore
...
@abc.Mapping.register #type: ignore
class Context:
    ...
    def __init__(
        ...
        self.name = name
        self.globals_keys = set() if globals is None else set(globals) #type: ignore
    ...
    keys = _dict_method_all(dict.keys) #type: ignore
    values = _dict_method_all(dict.values) #type: ignore
    items = _dict_method_all(dict.items) #type: ignore
...
class AsyncLoopContext(LoopContext):
    ...
    async def revindex(self) -> int:  # type: ignore
        return await self.length - self.index0 #type: ignore
...
class Macro:
    ...
    def __repr__(self) -> str:
        name = "anonymous" if self.name is None else repr(self.name) #type: ignore
...
def make_logging_undefined(
    ...
    def _log_message(undef: Undefined) -> None:
        logger.warning("Template variable warning: %s", undef._undefined_message) #type: ignore
